            ...
            <div class="row d-flex justify-content-center">
                <div class="col-xl-10">
                    <div class="card-box">
                        ...
                        <form action="{{route('datlich.update',$data->id)}}"  enctype="multipart/form-data"
                            method="post">
                            @csrf
                            {!! method_field('patch') !!}
                            <div class="row">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2 @error('idkhachhang') border-error @enderror" name="idkhachhang">
                                            ...
                                            @foreach ($KhachHang as $item)
                                                <option 
                                                   value="{{$item->id}}" <?php echo ($data->idkhachhang == $item->id) ? 'selected' : '';?>>{{$item->name}}
                                                </option>
                                            @endforeach
                                        </select>
                                        @if ($errors->has('idkhachhang'))
                                            <ul class="parsley-errors-list filled" id="parsley-id-7" aria-hidden="false"><li class="parsley-required">{{$errors->get('idkhachhang')[0]}}</li></ul>
                                        @endif
                                    </div>
                                </div>
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2 @error('idnhanvien') border-error @enderror" name="idnhanvien">
                                            ...
                                            @foreach ($NhanVien as $item)
                                                <option 
                                                    value="{{$item->id}}" <?php echo ($data->idnhanvien == $item->id) ? 'selected' : '';?>>{{$item->name}}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        ...
                                        <input type="date" class="form-control" value="thoigiandat" name="thoigiandat" id="">
                                        @if ($errors->has('thoigiandat'))
                                            <ul class="parsley-errors-list filled" id="parsley-id-7" aria-hidden="false">
                                            <li class="parsley-required">{{$errors->get('iddanhmuc')[0]}}</li></ul>
                                        @endif
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <label>Chọn dich vụ :</label> <br>
                                            @foreach ($DichVu as $item)
                                             <div class="form-check ml-1">
                                                <input type="checkbox" name="iddichvu" class="form-check-input" id="exampleCheck1" value={{(int)$item->id}} {{($data->name == $item->name ) ? 'checked' : '' }}>
                                                <label class="form-check-label" for="exampleCheck1">{{$item->name}}</label>
                                              </div>
                                            @endforeach
                                            
                                        @if ($errors->has('iddichvu'))
                                            <ul class="parsley-errors-list filled" id="parsley-id-7" aria-hidden="false"><li class="parsley-required">{{$errors->get('iddichvu')[0]}}</li></ul>
                                        @endif
                                    </div>
                                </div>
                            </div>
